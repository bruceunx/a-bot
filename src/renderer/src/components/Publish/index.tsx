...
export default function Publisher() {
  ...
  const { groups, accounts, refreshData } = useAccountStore();
  ...
  const filteredAccounts = useMemo(() => {
    return accounts.filter((acc) => {
      ...
      const matchesPlatform =
        filterPlatform === "All" || acc.platform === (filterPlatform as string);
      const matchesGroup =
        filterGroup === "All" ||
        acc.groups.some((group) => group.name === filterGroup);
      return matchesSearch && matchesPlatform && matchesGroup;
    });
  }, [accountSearch, filterPlatform, filterGroup, accounts]);
          ...
          <div className="p-4 bg-base-200/50 flex flex-wrap gap-2 items-center">
            ...
            >
              ...
              {groups
                .filter((g) => g.name !== "All")
                .map((g) => (
                  <option key={g.name} value={g.name} className="uppercase">
                    {g.name}
                  </option>
                ))}
            </select>
            ...
            >
              ...
              {Object.values(Platform).map((platform) => (
                <option key={platform} value={platform} className="uppercase">
                  {platform}
                </option>
              ))}
            </select>
